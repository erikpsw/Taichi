...
P=np.array([[30,0],[60,10],[80,30],[90,60],[90,90]])
U=np.array([0,0,0,0,0.5,1,1,1,1])
...
print(deBoorcox(P,0.1,k,0.25,U))
    
N=30
print(np.array([deBoorcox(P,2,j/N,U) for j in range(N+1)]).reshape(N+1,2))
...
if(mode==1):
    Y=np.array([deBoorcox(P,2,j/N,U) for j in range(N+1)]).reshape(N+1,2)
while gui.running:

...
p_RR = 0.7#轮盘赌概率
brightness=3
light_pos=ti.Vector([canvas_width/2,canvas_height,wall_distance/2])
            ...
            while(hit_times<=max_depth and not hit_light):
                if(ti.random()>p_RR):
                    ...
                    for k in ti.static(range(1,len(hierarchy))):#对所有物体求交
                        ans=hierarchy[k].get_hit_info(start_pos,light_dir)
                        if(ans[0]<t_min):
                            is_block=True
                    ...
                    if(index!=len(hierarchy)):#击中了
                        for k in ti.static(range(len(hierarchy))):
                            if(k==index):#小技巧来得到击中物体的索引
                                if(not is_block):
                                    color=proportion*color*brightness+(1-proportion)*hierarchy[k].color*hierarchy[k].hit_cos(ray_dir,j_hat)
                                else:
                                    color=proportion*color+(1-proportion)*hierarchy[k].color*hierarchy[k].hit_cos(ray_dir,j_hat)
                                if(hit_times==max_depth+1 and not hit_light):
                                    color=ti.Vector([0.,0.,0.])

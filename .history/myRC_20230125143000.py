...
def build_sence():
    ...
    Hierarchy.append(Sphere(ti.Vector([1.5*u,0.2*u,0.5*u]),0.2*u,ti.Vector([0.4, 0.6, 0.4]),2))
                    ...
                    if(index!=len(hierarchy)):#击中了
                        for k in ti.static(range(len(hierarchy))):
                            if(k==index):#小技巧来得到击中物体的索引
                                if(hit_times==1):
                                    if(not is_block):
                                        if(hierarchy[k].__class__.__name__=="rect"):
                                            light_cos=ti.math.dot(light_dir,hierarchy[k].rect_normal)/(ray_dir.norm())
                                            color=brightness*hierarchy[k].color*light_cos
                                        if(hierarchy[k].__class__.__name__=="Sphere"):
                                            light_cos=ti.math.dot(light_dir,j_hat)/(ray_dir.norm())
                                            color=brightness*hierarchy[k].color*light_cos
                                    else:

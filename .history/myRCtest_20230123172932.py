...
def build_sence():
    ...
    #ceil
    Hierarchy.append(rect(ti.Vector([canvas_width/2,canvas_height,wall_distance/2]),DOWN,canvas_width,wall_distance,RIGHT,ti.Vector([0.6, 0.0, 0.0])))          
    #ground
    ...
    if(cnt<=1):
        for i,j in canvas:
            ...
            for k in ti.static(range(len(hierarchy))):
                ...
                ans=hierarchy[k].get_t(camera_pos,ray_dir)
                if(ans[0]<t_min):
                    t_min=ans[0]
                    color-=color

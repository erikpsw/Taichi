...
@ti.func#重心坐标
def barycentric_coor(rect_width,rect_height,rect_pos,vec):
    list=[]
    for i in range(2):
        for j in range(2):
            #计算四个角对应的向量
            list.append(rect_pos[0]+((-1)**i)*rect_width/2,rect_pos[1]+((-1)**j)*rect_height/2)
    for i,j
def change_axes(vec,i_hat,j_hat):
    list=[i_hat,j_hat]
    A=ti.Matrix([[0.,0.],[0.,0.]])
...
def rect_render():
    for i,j in canvas:
        ...
        i_hat=ti.Vector([-1.,0.,-1.]).normalized()#指定长的方向向量

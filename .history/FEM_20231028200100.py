...
pos_list=[]
for i in np.linspace(0.3,0.4,N + 1,dtype=np.float32):
    for j in np.linspace(0.3,0.4,N + 1,dtype=np.float32):
       pos_list.append([i,j]) 
pos.from_numpy(np.array(pos_list))
f2v_list=[]
for i in range(N):
    for j in range(N):
        f2v_list.append([i*N+1+j,(i+1)*N+j,i*N+j])
        f2v_list.append([i*N+1+j,(i+1)*N+j,(i+1)*N+1+j])

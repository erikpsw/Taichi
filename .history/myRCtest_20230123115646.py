...
camera_pos=ti.Vector([canvas_width/2,canvas_height/2,-camera_distance],ti.f32).to_numpy()
...
rect_pos=ti.Vector([canvas_width/2,canvas_height/2,3*u]).to_numpy()#中心点位置
rect_normal=np.array([1.,0.,0.])
...
def change_axes(vec,i_hat,j_hat):
    ...
    if(list[0][2]==0):
        ...
        for i in ti.static(range(2)):
            for j in ti.static(range(2)):
                A[j,i]=list[i].all()[[1,2]][j]
        return np.linalg.solve(A,vec[[1,2]])

i_hat=np.array([0,1.,-1.])

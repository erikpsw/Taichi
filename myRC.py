...
class rect:
    ...
    @ti.func
    def get_reflex_info(self,j_hat,hit_pos,ray_dir):
        ...
        return (-ans[0]*self.i_hat-ans[1]*j_hat+self.rect_normal)#反射
@ti.func
def build_sence():
    Hierarchy=[]
            ...
            if(index!=len(hierarchy)):#小技巧来得到索引
                for k in ti.static(range(len(hierarchy))):
                    if(k==index):
                        ...
                        start_pos=hit_pos
                        ray_dir=hierarchy[k].get_reflex_info(j_hat,hit_pos,ray_dir)
            else:break
